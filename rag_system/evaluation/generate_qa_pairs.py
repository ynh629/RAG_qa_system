...
用法：
    ...
    # 自定义输入/输出/每块条数
    python -m rag_system.evaluation.generate_qa_pairs \
        --input rag_system/data/structured_segments.json \
        --output rag_system/data/qa_pairs_raw.json \
        --per-chunk 5

    # 只处理部分块（断点续跑 / 分批生成）
    python -m rag_system.evaluation.generate_qa_pairs --start 0 --end 4
"""
import argparse
import json
import os
...
# 数据文件路径（默认输入/输出均位于 rag_system/data/）
DEFAULT_INPUT = settings.RECURSIVE_JSON
DEFAULT_OUTPUT = os.path.join(settings.DATA_DIR, "qa_pairs_raw.json")
