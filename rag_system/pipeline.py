...
PDF_PATH = os.path.join(BASE_DIR, "文本解析", "年报.pdf")
STRUCTURED_JSON = os.path.join(DATA_DIR, "structured_segments.json")
RECURSIVE_JSON = os.path.join(DATA_DIR, "chunks_recursive.json")
SEMANTIC_JSON = os.path.join(DATA_DIR, "chunks_semantic.json")
...
def step1_parse_pdf(pdf_path: str = PDF_PATH, force: bool = False) -> str:
    ...
    from parse_pdf import parse_pdf_to_segments
...
def step3_apply_split(method: str, output_json: str, **kwargs) -> str:
    ...
    from advanced_splitting import apply_advanced_splitting
...
def step4_run_qa(data_json: str) -> None:
    ...
    from qa_system import run_qa_pipeline
